...
Rscript --vanilla RUVseq_with_fGSEA.R \
    contrast_id=$contrast_id contrast_var=$contrast_var donor_col=$donor_col control_grp=$control_grp experimental_grp=$experimental_grp \
    subset_on1=$subset_on1 subset_on2=$subset_on2 subset1=$subset1 subset2=$subset2 \
    covariates=$covariates correlate_vars=$correlate_vars indir=$indir outdir=$outdir metadata=$metadata \
    scale=$scale file_pattern=$file_pattern assay=$assay
